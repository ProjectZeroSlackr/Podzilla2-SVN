...
void pz_set_time_from_file(void)
{
	...
	stat( "/opt/Zillae/Podzilla2-SVN/Conf/podzilla2-svn.conf", &statbuf );
...
{
#ifdef IPOD
	close (open ("/opt/Zillae/Podzilla2-SVN/Conf/podzilla2-svn.conf", O_WRONLY));
#endif
}
...
debug_handler (int sig) 
{
    ...
    int i;
    FILE *f = fopen ("/opt/Zillae/Podzilla2-SVN/Misc/podzilla2-svn.oops", "w");
...
#ifdef IPOD
#define CONFIG_FILE "/opt/Zillae/Podzilla2-SVN/Conf/podzilla2-svn.conf"
#else
...
#define SET(x) pz_get_setting(pz_global_config,x)
	...
	if (!SET(COLORSCHEME))
		pz_set_string_setting (pz_global_config,COLORSCHEME, "familiar.cs");
	...
	pz_load_font (&ttk_menufont, "GPalatino", MENU_FONT, pz_global_config);
